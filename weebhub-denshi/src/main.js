...
const DEFAULT_UPDATE_FEED_URL = "https://github.com/5rahim/weebhub/releases/latest/download"
...
function getDesktopServerPort() {
    ...
    const envPort = Number.parseInt(process.env.WEEBHUB_SERVER_PORT || "", 10)
...
function createTray() {
    ...
    const contextMenu = Menu.buildFromTemplate([{
        ...
        id: "quit", label: "Quit WeebHub", click: () => {
            cleanupAndExit()
        }
    }
    ])

    tray.setToolTip("WeebHub")
...
async function launchWeebHubServer(isRestart) {
    return new Promise((resolve, reject) => {
        ...
        if (process.platform === "win32") {
            binaryName = "weebhub-server-windows.exe"
        } else if (process.platform === "darwin") {
            const arch = process.arch === "arm64" ? "arm64" : "amd64"
            binaryName = `weebhub-server-darwin-${arch}`
        } else if (process.platform === "linux") {
            const arch = process.arch === "arm64" ? "arm64" : "amd64"
            binaryName = `weebhub-server-linux-${arch}`
        }
        ...
        if (_development) {
            ...
            if (process.env.TEST_DATADIR) {
                ...
                const devDataDir = path.join(app.getPath("appData"), "WeebHub-dev")
        ...
        serverProcess.on("close", (code) => {
            ...
            if (!startupResolved && !isShutdown) {
                ...
                if (crashScreen && !crashScreen.isDestroyed()) {
                    crashScreen.show()
                    crashScreen.webContents.send("crash", `WeebHub server process terminated with status: ${code}. Closing in 10 seconds.`)
...
async function restartWeebHubServer() {
    ...
    serverRestartPromise = (async () => {
        ...
        await launchWeebHubServer(true)
    })().finally(() => {
...
async function fetchGithubStatus() {
    try {
        ...
        const response = await net.fetch("https://weebhub.app/api/github-status", {
            signal: controller.signal
        })
        clearTimeout(timeoutId)
        ...
        if (data.status === "down") {
            ...
            return { ok: false, fallback: data.fallback || "weebhub" }
        }
...
app.whenReady().then(async () => {
    ...
    if (currentUpdateChannel === "weebhub_nightly") {
        updateConfig.url = "https://weebhub.app/api/updates/nightly/"
        updateConfig.allowPrerelease = true
    } else if (currentUpdateChannel === "weebhub") {
        updateConfig.url = "https://weebhub.app/api/updates/stable/"
    ...
    try {
        ...
        await launchWeebHubServer(false)
    ...
    ipcMain.on("restart-server", () => {
        ...
        restartWeebHubServer().catch(console.error)
    })
