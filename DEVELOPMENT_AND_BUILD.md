...
* WeebHub Denshi: Built with a custom [Electron](https://www.electronjs.org/)/Chromium to support more codecs
...
2. After the build completes, a new `out` directory will be created inside `weebhub-web`.
...
1. **Windows (System Tray)**:
   ```bash
   set CGO_ENABLED=1
   go build -o weebhub.exe -trimpath -ldflags="-s -w -H=windowsgui -extldflags '-static'"
   ```
...
3. **Automated Type Generation**:
   ...
   - Types for the frontend are generated in:
     - `weebhub-web/api/generated/types.ts`
     - `weebhub-web/api/generated/endpoint.types.ts`
     - `weebhub-web/api/generated/hooks_template.ts`
...
AniList mock fixtures are read-only during normal test runs. Set `WEEBHUB_TEST_RECORD_ANILIST_FIXTURES=true` when you intentionally want missing or refreshed fixtures written back to the repository.
...
- It defaults to refreshing `./internal/api/anilist` and sets `WEEBHUB_TEST_RECORD_ANILIST_FIXTURES=true` for the test process.
