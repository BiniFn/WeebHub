...
- ✨ Torrent Search: Search from multiple providers at once
  - WeebHub can now aggregate torrent results from multiple providers
...
- ⚡️ Online streaming: Option to automatically cycle through providers
  - WeebHub can now try providers until a working one is found
...
- ⚡️ Added Update Channels (Experimental)
  - Github (Default), WeebHub (GitHub alternative), WeebHub Canary (Alpha/Beta releases)
...
- ⚡️ New Transcoding/Direct Play media player
  - Video playback using transcoding/direct play will now use WeebHub's custom player (VideoCore)
...
- ✨️ Nakama: Cloud Rooms (Public Beta)
  ...
  - Communication between the host and peers is managed by WeebHub's Rooms API
...
- 📝 BREAKING: WeebHub Desktop is now deprecated. Download the new desktop client https://weebhub.app/download
- 🎉 WeebHub Denshi: New desktop client & built-in player
  - WeebHub Denshi (based on Electron) replaces WeebHub Desktop (based on Tauri)
...
- 🎉 Custom Sources: New extension type for adding custom media
  - WeebHub now no longer limited to AniList!
...
- ⚡️ Migrated to WeebHub's own anime metadata API
...
- 🎉 New feature: Nakama - Communication between WeebHub instances
  - You can now communicate with other WeebHub instances over the internet
...
- 🎉 Nakama: Share your anime library (Alpha)
  - Share your local anime library with other WeebHub instances or consume your remote library
- ✨ Local account
  - By default, WeebHub no longer requires an AniList account and stores everything locally
- ✨ Server password
  - Lock your exposed WeebHub instance by adding a password in your config file
...
- 🎉 Plugins: A powerful new way to extend and customize WeebHub
...
- 🏗️ Moved 'watch continuity' setting to 'WeebHub' tab
...
- ⚡️ Desktop: Close to minimize to tray
  ...
  - Exit the app by right-clicking the tray icon and selecting 'Quit WeebHub'
...
- 🎉 Windows System Tray App
  - WeebHub now runs as a system tray app on Windows, offering quick and easy access
...
- 🚀 The web interface is now bundled with the binary
  - WeebHub now ships without the `web` directory
  - This should solve issues with auto updates on Windows
- 🎉 Media streaming: Direct play support
  - WeebHub will now, automatically attempt to play media files directly without transcoding if the client supports the codecs
...
- ✨ Self update (Experimental)
  - Update WeebHub to the latest version directly from the web UI
...
- ✨ Docker image
  - WeebHub is now available as a Docker image. Check DOCKER.md for more information
- ⚡️ Added '--truewd' flag to force to WeebHub use the binary's directory as the working directory
...
- ✨ Command line flags
    - Use `--datadir` to override the default data directory and use multiple WeebHub instances
...
- ⚡️Improved auto file section for torrent downloads
    - WeebHub can now select the right episode files in multi-season batches, and will only fail when it can’t tell seasons apart
...
- 🏗️ Handle runtime errors gracefully
  - WeebHub will now try to recover from runtime errors and display the stack trace
...
- ✨ Added update notifications
  - WeebHub will now check for updates on startup and notify you if a new version is available (can be disabled in settings)
...
- 🎉 New feature: Track progress on MyAnimeList
  - You can now link your MyAnimeList account to WeebHub and automatically update your progress
...
- 🎉 New feature: Automatically download new episodes
  ...
  - WeebHub will automatically parse the Nyaa RSS feed and download new episodes based on your rules
...
- ⚡️ Improved handling of AniList rate limits
  - WeebHub will now pause and resume requests when rate limits are reached without throwing errors. This fixes the largest issue pertaining to scanning.
