...
export function ServerSettings(props: ServerSettingsProps) {
    ...
    const confirmDialog = useConfirmationDialog({
        ...
        description: "This will upload your local WeebHub collection to your AniList account. Are you sure you want to proceed?",
        actionText: "Upload",
        ...
        <div className="space-y-4">
            ...
            <SettingsCard>
                ...
                <Field.Switch
                    ...
                    help="If enabled, WeebHub will remember your watch progress and resume from where you left off."
                    moreHelp="Only applies to desktop and integrated players."
                ...
                <div data-settings-enable-extension-secure-mode>
                    <Field.Switch
                        ...
                        help="If enabled, WeebHub will prompt you for confirmation whenever an extension tries to perform a sensitive action, even if permissions have been granted."
            ...
            <SettingsCard title="Offline mode" description="Only available when authenticated with AniList.">
                ...
                <Field.Switch
                    ...
                    help="If enabled, WeebHub will automatically save all media you're currently watching/reading for offline use."
            ...
            <SettingsCard title="App">
                ...
                <Field.Switch
                    ...
                    help="If enabled, WeebHub will stop caching AniList requests to disk."
                    moreHelp="By default, all requests made to AniList are cached. This allows WeebHub to keep being usable when AniList goes down. The cache directory is modifiable in the config file."
                ...
                {!f.watch("disableCacheLayer") && (
                    <div className="space-y-1 pl-4 border-l border-[--border] ml-2">
                        <Switch
                            ...
                            moreHelp="WeebHub will use cached data instead of making API requests."
                        />
                    </div>
                )}
                <Separator />
                <Field.Switch
                    ...
                    help="If enabled, WeebHub will use an alternative source to fetch episode metadata."
                ...
                <Field.Switch
                    ...
                    help={__isElectronDesktop__ ? (<span className="flex gap-2 items-center">
                        ...
                        <span>If enabled, new releases won't be displayed. WeebHub Denshi may still auto-update in the background.</span>
                    </span>) : "If enabled, WeebHub will not check for new releases."}
                    moreHelp={__isElectronDesktop__ ? "You cannot disable auto-updates for WeebHub Denshi." : undefined}
                    icon={<TbDownloadOff className="" />}
                />
                <Field.Select
                    label="Update Channel"
                    name="updateChannel"
                    help={__isElectronDesktop__ ? "Also applies to WeebHub Denshi auto-updates." : ""}
                    options={[
                        { label: "GitHub (Default)", value: "github" },
                        { label: "WeebHub", value: "weebhub" },
                        { label: "WeebHub (Canary)", value: "weebhub_nightly" },
                    ]}
                />
                {serverStatus?.settings?.library?.updateChannel === "weebhub" && (
                    <Alert intent="info" description="You are currently using a release channel hosted on WeebHub." />
                )}
                {serverStatus?.settings?.library?.updateChannel === "weebhub_nightly" && (
                    <Alert
                        intent="warning"
                        description="You are currently using the canary release channel hosted on WeebHub. This channel may receive unstable updates without much testing."
                    />
                )}
                <Separator />
                <Field.Switch
