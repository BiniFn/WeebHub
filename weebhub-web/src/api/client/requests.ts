...
export async function buildSeaQuery<T, D extends any = any>(
    ...
    if (password) {
        headers["X-WeebHub-Token"] = password
    }

    const clientId = getClientId()
    const clientIdProof = getClientIdProof()
    if (clientId) {
        headers["X-WeebHub-Client-Id"] = clientId
    }
    if (clientIdProof) {
        headers["X-WeebHub-Client-Id-Proof"] = clientIdProof
    }
    if (__clientPlatform__) {
        headers["X-WeebHub-Client-Platform"] = __clientPlatform__
    }
...
function syncClientIdFromHeader(headers: unknown) {
    ...
    const clientId = getHeaderValue(headers, "x-weebhub-client-id")
    const clientIdProof = getHeaderValue(headers, "x-weebhub-client-id-proof")
