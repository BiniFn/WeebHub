...
var (
	...
	fallbackGithubUrl    = "https://api.github.com/repos/5rahim/weebhub/releases/latest"
...
func (u *Updater) GetReleaseName(version string) string {
	...
	return fmt.Sprintf("weebhub-%s_%s_%s.%s", version, oos, arch, ext)
}
...
func (u *Updater) fetchLatestRelease(channel string) (*Release, error) {
	...
	switch channel {
	case "weebhub_nightly":
		apiRelease, err := u.fetchLatestReleaseFromApi(weebhubNightlyUrl)
		if err != nil {
			return nil, err
		}
		release = apiRelease
	case "weebhub":
		apiRelease, err := u.fetchLatestReleaseFromApi(weebhubStableUrl)
		if err != nil {
			return nil, err
		}
		release = apiRelease
...
func (u *Updater) fetchGithubStatus() (string, bool) {
	...
	if res.Status == "down" {
		...
		return cmp.Or(res.Fallback, "weebhub"), false
	}
