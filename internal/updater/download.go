...
func (u *Updater) decompressZip(archivePath string, folderName string) (dest string, err error) {
	topFolderName := "weebhub-" + u.LatestRelease.Version
	if folderName != "" {
		topFolderName = folderName
	}
	// "/weebhub-repo/weebhub-v1.0.0.zip" -> "/weebhub-repo/weebhub-1.0.0/"
	dest = filepath.Join(filepath.Dir(archivePath), topFolderName) // "/weebhub-repo/weebhub-v1.0.0"
...
//   - "weebhub-v1.zip" -> "/weebhub-repo/weebhub-v1.zip"
