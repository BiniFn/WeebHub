...
const (
	tempReleaseDir = "weebhub_new_release"
...
func NewSelfUpdater() *SelfUpdater {
	...
	ret.tmpExecutableName = "weebhub.exe.old"
	switch runtime.GOOS {
	case "windows":
		ret.tmpExecutableName = "weebhub.exe.old"
	default:
		ret.tmpExecutableName = "weebhub.old"
	}
...
func (su *SelfUpdater) recover(assetUrl string) {
	...
	su.logger.Error().Msg("selfupdate: Failed to install update. Update downloaded to 'weebhub_new_release'")
}

func getExePath() string {
	exe, err := os.Executable() // /path/to/weebhub.exe
	if err != nil {
		return ""
	}
	exePath, err := filepath.EvalSymlinks(exe) // /path/to/weebhub.exe
	if err != nil {
		return ""
	}
...
func (su *SelfUpdater) Run() error {
	...
	// Download the asset to exeDir/weebhub_tmp
	...
	// weebhub.exe + /backup_restore_if_failed/weebhub.exe
	...
	for _, file := range files {
		...
		if err != nil && (file == "weebhub" || file == "weebhub.exe") {
			renamingFailed = true
	...
	// Remove weebhub.exe.old and LICENSE.old
	for _, file := range files {
