...
func newUpdaterTestFixture(t *testing.T) *updaterTestFixture {
	...
	release := &Release{
		...
		Assets: []ReleaseAsset{
			{
				Url:                server.URL + "/assets/weebhub-3.5.2_Windows_x86_64.zip",
				Id:                 1,
				NodeId:             "asset-zip",
				Name:               "weebhub-3.5.2_Windows_x86_64.zip",
				ContentType:        "application/zip",
				Uploaded:           true,
				Size:               int64(len(zipArchive)),
				BrowserDownloadUrl: server.URL + "/assets/weebhub-3.5.2_Windows_x86_64.zip",
			},
			{
				Url:                server.URL + "/assets/weebhub-3.5.2_MacOS_arm64.tar.gz",
				Id:                 2,
				NodeId:             "asset-tar",
				Name:               "weebhub-3.5.2_MacOS_arm64.tar.gz",
				ContentType:        "application/gzip",
				Uploaded:           true,
				Size:               int64(len(tarGzArchive)),
				BrowserDownloadUrl: server.URL + "/assets/weebhub-3.5.2_MacOS_arm64.tar.gz",
			},
		},
	}
	...
	mux.HandleFunc("/assets/weebhub-3.5.2_Windows_x86_64.zip", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/zip")
		_, _ = w.Write(zipArchive)
	})
	mux.HandleFunc("/assets/weebhub-3.5.2_MacOS_arm64.tar.gz", func(w http.ResponseWriter, r *http.Request) {
...
func (f *updaterTestFixture) apply(t *testing.T) {
	...
	oldWeebHubStableURL := weebhubStableUrl
	oldWeebHubNightlyURL := weebhubNightlyUrl
	...
	weebhubStableUrl = f.server.URL + "/api/updates/stable/stable_server.json"
	weebhubNightlyUrl = f.server.URL + "/api/updates/nightly/nightly_server.json"

	t.Cleanup(func() {
		websiteUrl = oldWebsiteURL
		fallbackGithubUrl = oldFallbackGithubURL
		githubCheckUrl = oldGithubCheckURL
		weebhubStableUrl = oldWeebHubStableURL
		weebhubNightlyUrl = oldWeebHubNightlyURL
	})
}
...
func mustCreateZipArchive(t *testing.T) []byte {
	...
	files := map[string]string{
		"bin/weebhub.exe": "windows-binary",
