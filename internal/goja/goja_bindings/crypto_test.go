...
func TestGojaCryptoAES(t *testing.T) {
	t.Run("random iv round trip", func(t *testing.T) {
		...
		require.Equal(t, "weebhub", obj.Get("decrypted").String())
	})
	...
	t.Run("invalid iv length returns an error string", func(t *testing.T) {
		...
		val, err := vm.RunString(`
            (() => {
                try {
                    CryptoJS.AES.encrypt("weebhub", CryptoJS.enc.Utf8.parse("secret key"), {
	...
	tests := []struct {
		...
		},
		{
			...
			script: `
				(() => {
					try {
						CryptoJS.AES.encrypt("weebhub", CryptoJS.enc.Utf8.parse("secret key")).toString("bad");
						return "unexpected success";
					} catch (e) {
						return String(e);
					}
