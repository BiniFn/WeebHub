...
func addQuitItem() {
	...
	mQuit := systray.AddMenuItem("Quit WeebHub", "Quit the whole app")
	mQuit.Enable()
	go func() {
		<-mQuit.ClickedCh
...
func onReady(webFS *embed.FS, app *core.App, flags core.WeebHubFlags, selfupdater *updater.SelfUpdater) func() {
	return func() {
		systray.SetTemplateIcon(icon.Data, icon.Data)
		systray.SetTitle(fmt.Sprintf("WeebHub v%s", constants.Version))
		systray.SetTooltip(fmt.Sprintf("WeebHub v%s", constants.Version))
		log.Trace().Msg("systray: App is ready")

		// Menu items
		systray.AddMenuItem("WeebHub v"+constants.Version, "WeebHub version")
