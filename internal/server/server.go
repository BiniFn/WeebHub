...
import (
	...
	"weebhub/internal/cron"
	"weebhub/internal/handlers"
	"weebhub/internal/updater"
	"weebhub/internal/util"
	"weebhub/internal/util/crashlog"
	"time"

	"github.com/rs/zerolog/log"
)

func startApp(embeddedLogo []byte) (*core.App, core.WeebHubFlags, *updater.SelfUpdater) {
	// Print the header
	core.PrintHeader()
	...
	// Create log file
	logFilePath := filepath.Join(app.Config.Logs.Dir, fmt.Sprintf("weebhub-%s.log", time.Now().Format("2006-01-02_15-04-05")))
	// Open the log file
...
func startAppLoop(webFS *embed.FS, app *core.App, flags core.WeebHubFlags, selfupdater *updater.SelfUpdater) {
