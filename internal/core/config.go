...
type Config struct {
	...
	Server  struct {
		...
		Port            int
		Offline         bool
		UseBinaryPath   bool // Makes $WEEBHUB_WORKING_DIR point to the binary's directory
		Systray         bool
		DoHUrl          string
		Password        string
...
func NewConfig(options *ConfigOptions, logger *zerolog.Logger) (*Config, error) {
	...
	if os.Getenv("WEEBHUB_DATA_DIR") != "" {
		definedDataDir = os.Getenv("WEEBHUB_DATA_DIR")
	}
	...
	if os.Getenv("WEEBHUB_SERVER_HOST") != "" {
		defaultHost = os.Getenv("WEEBHUB_SERVER_HOST")
	}
	if os.Getenv("WEEBHUB_SERVER_PORT") != "" {
		var err error
		defaultPort, err = strconv.Atoi(os.Getenv("WEEBHUB_SERVER_PORT"))
		if err != nil {
			return nil, fmt.Errorf("invalid WEEBHUB_SERVER_PORT environment variable: %s", os.Getenv("WEEBHUB_SERVER_PORT"))
		}
	}
	...
	// Set WeebHub's default custom environment variables
	...
	viper.SetDefault("database.name", "weebhub")
	viper.SetDefault("web.assetDir", "$WEEBHUB_DATA_DIR/assets")
	viper.SetDefault("cache.dir", "$WEEBHUB_DATA_DIR/cache")
	viper.SetDefault("cache.transcodeDir", "$WEEBHUB_DATA_DIR/cache/transcode")
	viper.SetDefault("manga.downloadDir", "$WEEBHUB_DATA_DIR/manga")
	viper.SetDefault("manga.localDir", "$WEEBHUB_DATA_DIR/manga-local")
	viper.SetDefault("logs.dir", "$WEEBHUB_DATA_DIR/logs")
	viper.SetDefault("offline.dir", "$WEEBHUB_DATA_DIR/offline")
	viper.SetDefault("offline.assetDir", "$WEEBHUB_DATA_DIR/offline/assets")
	viper.SetDefault("extensions.dir", "$WEEBHUB_DATA_DIR/extensions")
	...
	if (flags.Host != "" || os.Getenv("WEEBHUB_SERVER_HOST") != "") && existingHost != defaultHost {
		viper.Set("server.host", defaultHost)
		hostChanged = true
	}
	if (flags.Port != 0 || os.Getenv("WEEBHUB_SERVER_PORT") != "") && existingPort != defaultPort {
	...
	cfg.Data.WorkingDir = os.Getenv("WEEBHUB_WORKING_DIR")

	if cfg.Server.Tls.Enabled && (cfg.Server.Tls.CertPath == "" || cfg.Server.Tls.KeyPath == "") {
		viper.SetDefault("server.tls.certPath", "$WEEBHUB_DATA_DIR/certs/cert.pem")
		viper.SetDefault("server.tls.keyPath", "$WEEBHUB_DATA_DIR/certs/key.pem")
		_ = viper.WriteConfig()
...
func setDataDirEnv(dataDir string) error {
	...
	if os.Getenv("WEEBHUB_DATA_DIR") == "" {
		if err := os.Setenv("WEEBHUB_DATA_DIR", dataDir); err != nil {
			return err
		}
	}
...
func setWorkingDirEnv(useBinaryPath bool) error {
	...
	if err = os.Setenv("WEEBHUB_WORKING_DIR", filepath.FromSlash(wd)); err != nil {
		return err
	}
...
func initAppDataDir(definedDataDir string, logger *zerolog.Logger) (dataDir string, configPath string, err error) {
	...
	if definedDataDir != "" {
		...
		dataDir = filepath.Join(dataDir, "WeebHub")
	}
...
func loadLogo(embeddedLogo []byte, dataDir string) (err error) {
	...
	logoPath := filepath.Join(dataDir, "weebhub-logo.png")
