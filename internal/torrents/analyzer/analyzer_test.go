...
func TestNewAnalyzerInitializesFiles(t *testing.T) {
	root := t.TempDir()
	paths := []string{
		filepath.Join(root, "Season 1", "[WeebHub] Example Show - 01.mkv"),
		filepath.Join(root, "Season 1", "[WeebHub] Example Show - 02.mkv"),
	}
...
func TestAnalyzeTorrentFilesReturnsErrorWhenPlatformRefAbsent(t *testing.T) {
	analyzer := NewAnalyzer(&NewAnalyzerOptions{
		Filepaths: []string{filepath.Join(t.TempDir(), "[WeebHub] Example Show - 01.mkv")},
		Media:     &anilist.CompleteAnime{ID: 42},
	})
...
func newAnalysisFixture(t *testing.T) (*Analysis, []*File) {
	...
	root := t.TempDir()
	files := []*File{
		newAnalyzedFile(filepath.Join(root, "[WeebHub] Example Show - 01.mkv"), 0, 42, 1, anime.LocalFileTypeMain, "1"),
		newAnalyzedFile(filepath.Join(root, "[WeebHub] Example Show - OVA.mkv"), 1, 42, 0, anime.LocalFileTypeSpecial, "S1"),
		newAnalyzedFile(filepath.Join(root, "[WeebHub] Other Show - 01.mkv"), 2, 7, 1, anime.LocalFileTypeMain, "1"),
		newAnalyzedFile(filepath.Join(root, "[WeebHub] Example Show - 03.mkv"), 3, 42, 3, anime.LocalFileTypeMain, "3"),
	}
