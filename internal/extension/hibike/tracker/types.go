...
type (
	Settings struct {
		...
		// SupportsBidirectionalSync indicates if WeebHub can pull data from the tracker
	...
	MediaEntry struct {
		...
		// MediaId is the AniList or custom source media ID (WeebHub ID).
		...
		// ExternalId
		//	When Pulling: The extension MUST populate this.
		//	When Pushing: WeebHub will populate this (using ResolveExternalId).
	...
	Provider interface {
		...
		// ResolveExternalId finds the tracker-specific ID for a given WeebHub media entry.
		// WeebHub calls this BEFORE calling PushEntry to ensure MediaEntry.ExternalId is populated.
...
const (
	...
	DiffTypeLocalOnly    DiffType = "local-only"    // Exists in WeebHub, missing in Tracker
	DiffTypeRemoteOnly   DiffType = "remote-only"   // Exists in Tracker, missing in WeebHub
