...
func (h *Handler) HandleGetLogFilenames(c echo.Context) error {
	...
	if len(filenames) > 0 {
		...
		for _, filename := range filenames {
			if strings.HasPrefix(strings.ToLower(filename), "weebhub-") {
...
//	@desc This returns the content of the most recent weebhub- log file after flushing logs.
...
func (h *Handler) HandleGetLatestLogContent(c echo.Context) error {
	...
	for _, entry := range dirEntries {
		...
		name := entry.Name()
		if filepath.Ext(name) != ".log" || !strings.HasPrefix(strings.ToLower(name), "weebhub-") {
			continue
		}
...
func (h *Handler) HandleGetMemoryProfile(c echo.Context) error {
	...
	if heap {
		filename = fmt.Sprintf("weebhub-heap-profile-%s.pprof", timestamp)
		profile = pprof.Lookup("heap")
	} else if allocs {
		filename = fmt.Sprintf("weebhub-allocs-profile-%s.pprof", timestamp)
...
func (h *Handler) HandleGetGoRoutineProfile(c echo.Context) error {
	...
	timestamp := time.Now().Format("2006-01-02_15-04-05")
	filename := fmt.Sprintf("weebhub-goroutine-profile-%s.pprof", timestamp)
...
func (h *Handler) HandleGetCPUProfile(c echo.Context) error {
	...
	timestamp := time.Now().Format("2006-01-02_15-04-05")
	filename := fmt.Sprintf("weebhub-cpu-profile-%s.pprof", timestamp)
