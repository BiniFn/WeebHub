...
func (h *Handler) HandleNakamaProxyStream(c echo.Context) error {
	...
	if streamType == "debrid" {
		...
		req.Header.Set("X-WeebHub-Nakama-Token", h.App.Settings.GetNakama().RemoteServerPassword)
...
//
//	@summary creates a WeebHub Room and connects to it as host.
//	@desc This creates a room on the WeebHub Rooms relay server and connects as the host. Peers can then join using the returned URL.
