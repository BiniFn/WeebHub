...
func (m *Manager) RoomsAvailable() bool {
	resp, err := m.reqClient.R().
		Get(constants.WeebHubRoomsApiUrl + "/health")
	if err != nil {
		return false
	}
	...
	if createResp.Status != "healthy" && createResp.Version != constants.WeebHubRoomsVersion {
		return false
	}
...
// CreateAndJoinRoom creates a room using WeebHub Rooms API and connects as host
...
func (m *Manager) HandlePeerConnection(w http.ResponseWriter, r *http.Request) {
	...
	username := r.Header.Get("X-WeebHub-Nakama-Username")
	...
	peerID := r.Header.Get("X-WeebHub-Nakama-Peer-Id")
	if peerID == "" {
	...
	serverVersion := r.Header.Get("X-WeebHub-Nakama-Server-Version")
