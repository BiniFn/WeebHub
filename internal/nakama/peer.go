...
// isRoomURL checks if the URL is a WeebHub Rooms URL
...
// attemptDirectConnection makes a direct H2P connection to a WeebHub host
func (m *Manager) attemptDirectConnection(connCtx context.Context) error {
	...
	headers := http.Header{}
	headers.Set("X-WeebHub-Nakama-Token", m.settings.RemoteServerPassword)
	headers.Set("X-WeebHub-Nakama-Username", username)
	headers.Set("X-WeebHub-Nakama-Server-Version", constants.Version)
	headers.Set("X-WeebHub-Nakama-Peer-Id", peerID)
...
// attemptRoomConnection connects to a WeebHub Rooms relay as a peer
func (m *Manager) attemptRoomConnection(connCtx context.Context) error {
	...
	u, err := url.Parse(fmt.Sprintf("%s/%s/peer", constants.WeebHubRoomsApiWsUrl, roomId))
	if err != nil {
		return err
	}
	q := u.Query()
